...
from binBlkMask_codes.base_binBlkMask import attention_binaryBlkMat
from binBlkMask_codes.dense_binBlkMask import attention_dense_binaryBlkMat
from binBlkMask_codes.combined_binBlkMask import attention_combined_binBlkMask
from binBlkMask_codes.combine_wo_binShift import attention_combined_wo_binBlkMask
import numpy as np
...
maskMat = get_hash_causal_matrix(N_CTX, 32, 128)
...
# binary_blk_mask = create_blk_one_matrix(maskMat, 128, 32).to(dtype=torch.uint8)
# binary_blk_mask_T = create_blk_one_matrix(maskMat.T, 128, 32).to(dtype=torch.uint8)
# binary_blk_int32 = get_int32_from_binaryMat(binary_blk_mask)
# binary_blk_int32_T = get_int32_from_binaryMat(binary_blk_mask_T)
# binary_blk_int32 = binary_blk_int32.to("cuda")
# binary_blk_int32_T = binary_blk_int32_T.to("cuda")
...
# tri_out = attention_dense_binaryBlkMat(q, k, v, causal, sm_scale, maskMat, binary_blk_mask, binary_blk_mask_T,
#                                        num_ones, offset, num_ones_T, offset_T).half()

tri_out = attention_combined_wo_binBlkMask(q, k, v, causal, sm_scale, maskMat, binary_blk_mask, binary_blk_mask_T,
                                           num_ones, offset, num_ones_T, offset_T).half()

# tri_out = attention_combined_binBlkMask(q, k, v, causal, sm_scale, maskMat, binary_blk_int32, binary_blk_int32_T,
#                                              num_ones, offset, num_ones_T, offset_T).half()
