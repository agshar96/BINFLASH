'''
This file has the code for the binary block masking attention where we check 
transition from a 1 to 0 block to speed up processing in causal/dense block cases.
'''
import torch
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    last_blk = False # If IS_DENSEBLK is not set then this value will always be 0
    inside_process_loop = False# This is to check if we had any 1 binary block

    for start_n in range(lo, hi, BLOCK_N):
    # for start_n in range(lo, hi, BLOCK_N):
        cur_val = start_n // BLOCK_N
        start_n = tl.multiple_of(start_n, BLOCK_N)
        if last_blk == False or IS_DENSEBLK == 0: ## This is general case
            ...
            if inside_process_loop == True and binaryBlk_val == 0:
                last_blk = True
        else:
        ...
        if binaryBlk_val:
            ...
            if inside_process_loop == False and IS_DENSEBLK == 1:
                inside_process_loop = True
...
def _attn_bwd_dkdv(dk, dv,  #
    ...
    for blk_idx in range(num_steps):
        if last_blk == False or IS_DENSEBLK == 0:
...
class _attention_transition_binaryBlkMask(torch.autograd.Function):
    '''
...
attention_transition_binaryBlkMask = _attention_transition_binaryBlkMask.apply